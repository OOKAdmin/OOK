...
export default function IsectionFile(props) {
  ...
  const [dinputValue, setdInputValue] = useState(0);
  ...
  const [twinputValue, settwinputValue] = useState(0);
  ...
  const Runits = ['mm', 'cm', 'm'];
  const RconversionFactors = {
      mm: [1, 0.1, 0.001],
      cm: [10, 1, 0.01],
      m: [1000, 100, 1],
  };
  const [RinputValue, setRinputValue] = useState(0);
  const [RselectedUnit, setRinputValueSelectedUnit] = useState('mm');
  const [internalRinputValue, setInternalRinputValue] = useState(0); // Always in mm

  const handleRInputChange = (value) => {
      setRinputValue(value);
      const factor = RconversionFactors[RselectedUnit][0];
      setInternalRinputValue(parseFloat(value) * factor);
  };

  const handleRUnitChange = (unit) => {
      const newMetricValueInMM = parseFloat(RinputValue) * RconversionFactors[RselectedUnit][0];
      const convertedValue = newMetricValueInMM / RconversionFactors[unit][0];
      setRinputValueSelectedUnit(unit);
      setRinputValue(convertedValue.toFixed(5));
  };
  ...
  const R = parseFloat(internalRinputValue);
  ...
  useEffect(() => {
      ...
      const formattedValue = centroidYcSelectedUnit === 'm' 
  ...
  useEffect(() => {
      const value = b;
      if (!isNaN(value)) {
          const k1 = ((b * Math.pow(t, 3)) / 3) - (0.21 * Math.pow(t, 4)) - (0.0175 * (Math.pow(t, 8) / Math.pow(b, 4)))
          const k2 = (d * Math.pow(tw, 3)) / 3
          // const D = Math.pow((t -(-R)),2)
          // const D = R*tw
          const Dnumenator = (Math.pow((t - (-R)), 2) + R * tw + Math.pow(tw, 2) / 4)
          const Ddenomenator = 2 * R + t
          ...
          const alpha = (t1 / t2) * (0.15 + (0.1 * (R / t)))
          const K = 2 * k1 + k2 + (2 * alpha * Math.pow(D, 4))
          const TorsionalConstant = K.toFixed(3);
          const convertedValue = (TorsionalConstant * TorsionalConstantConversionFactors['mm⁴'][TorsionalConstantUnits.indexOf(TorsionalConstantSelectedUnit)]);
  ...
  }, [b, d, t, tw, R, TorsionalConstantSelectedUnit]);
  ...
  const [ImperialbinputValue, setImperialbInputValue] = useState(0);
  ...
  const [ImperialdinputValue, setImperialdInputValue] = useState(0);
  ...
  const [ImperialtwinputValue, setImperialTwInputValue] = useState(0);
  ...
  const ImperialRunits = ['in'];
  const ImperialRconversionFactors = {
      in: [1],
  };

  const [ImperialRinputValue, setImperialRInputValue] = useState(0);
  const [ImperialRSelectedValue, setImperialRSelectedUnit] = useState('in');

  const handleImperialRInputChange = (value) => {
      setImperialRInputValue(value);
  };

  const handleImperialRUnitChange = (unit) => {
      setImperialRSelectedUnit(unit);
      const ImperialRfactor = ImperialRconversionFactors[unit][ImperialRunits.indexOf(ImperialRSelectedValue)];
      setImperialRInputValue((parseFloat(ImperialRinputValue) / ImperialRfactor).toFixed(4));
  };
  ...
  const [Imperialt, setImperialt] = useState(0);
  ...
  const [ImperialSectionModulesSx, setImperialSectionModulesSx] = useState(0);
  ...
  // Handler for changing ImperialTorsionalConstant unit
  const handleImperialTorsionalConstantUnitChange = (unit) => {
      setImperialTorsionalConstantSelectedUnit(unit);
      const currentFactor = ImperialTorsionalConstantConversionFactors[unit][ImperialTorsionalConstantUnits.indexOf(ImperialTorsionalConstantSelectedUnit)];
      setImperialTorsionalConstant((prevImperialTorsionalConstant) => prevImperialTorsionalConstant * currentFactor);
  };
  const tImperial = parseFloat(Imperialt);
  const RImperial = parseFloat(ImperialRinputValue);
  const twImperial = parseFloat(ImperialtwinputValue);
  const bImperial = parseFloat(ImperialbinputValue);
  const dImperial = parseFloat(ImperialdinputValue);

  // Calculate ImperialTorsionalConstant based on MomentOfInertiaIy and CentroidXc
  const calculateImperialTorsionalConstant = () => {
      const k1 = ((bImperial * Math.pow(tImperial, 3)) / 3) - (0.21 * Math.pow(tImperial, 4)) - (0.0175 * (Math.pow(tImperial, 8) / Math.pow(bImperial, 4)))
      const k2 = (dImperial * Math.pow(twImperial, 3)) / 3
      // const D = Math.pow((t -(-R)),2)
      // const D = R*tw
      const Dnumenator = (Math.pow((tImperial - (-RImperial)), 2) + RImperial * twImperial + Math.pow(twImperial, 2) / 4)
      const Ddenomenator = 2 * RImperial + tImperial
      const D = Dnumenator / Ddenomenator
      if (tImperial < twImperial) {
          var t1 = tImperial
          var t2 = twImperial
      } else {
          var t1 = twImperial
      ...
      const alpha = (t1 / t2) * (0.15 + (0.1 * (RImperial / tImperial)))
      const K = 2 * k1 + k2 + (2 * alpha * Math.pow(D, 4))
      const TorsionalConstant = K.toFixed(3);
      const convertedValue = (TorsionalConstant * ImperialTorsionalConstantConversionFactors['in⁴'][ImperialTorsionalConstantUnits.indexOf(ImperialTorsionalConstantSelectedUnit)]);
      setImperialTorsionalConstant(convertedValue);
  };
  useEffect(() => {
      calculateImperialTorsionalConstant();
  });
  return (
      <>
          ...
          {props.MetricOrImperial === 'option1' && (
              <>
                  ...
                  <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      <h3 className='claculator-conversation-title'>Web Thickness (t<span className='LowerPowerminus2px torsinalConstantFormula'>w</span>) :</h3>
                      <div className='Calculator-Side-A'>
                          <input
                              className='calculator-input'
                              type="number"
                  ...
                  <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      ...
                      <div className='Calculator-Side-A'>
                          ...
                          <select
                              className='Calculator-select-option'
                              value={RselectedUnit}
                              onChange={(e) => handleRUnitChange(e.target.value)}
                          >
                              {Runits.map((unit) => (
                                  <option key={unit} value={unit}>
                                      {unit}
                  ...
                  <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      ...
                      <div className='Calculator-Side-A'>
                          <input
                              className='calculator-input'
                              type="number"
                              value={ImperialRinputValue}
                              onChange={(e) => handleImperialRInputChange(e.target.value)}
                          />
                          <select
                              className='Calculator-select-option'
                              value={ImperialRSelectedValue}
                              onChange={(e) => handleImperialRUnitChange(e.target.value)}
                          >
                              {ImperialRunits.map((unit) => (
                                  <option key={unit} value={unit}>
                                      {unit}
                                  </option>
                              ))}
